...
func persist(gcs *GCSclient, cssvc *CloudSpannerService, files []string) error {
	...
	chs := []chan *spanner.Mutation{}
	for i := 0; i < 4; i++ {
		chs = append(chs, make(chan *spanner.Mutation, 20000))
	}
	...
	case "Account":
		...
		for _, f := range filterStrings(files,
			...
			readersCh <- &processJob{
				...
				lookup: lookup,
				processFunction: func(j *processJob, chs []chan *spanner.Mutation) error {
					gocsv.UnmarshalBytesToCallback(j.files[j.mapping["accounts"]], func(a *dbAccount) error {
						...
						return appendMutation(chs[0], "Account", a)
					})
					j.wgs["Account"].Done()
					return nil
				},
			...
			readersCh <- &processJob{
				...
				processFunction: func(j *processJob, chs []chan *spanner.Mutation) error {
					...
					for _, o := range orders {
						...
						err = appendMutation(chs[0], "Order", o)
						if err != nil {
							return err
						}
					}
					...
					for _, ol := range ols {
						...
						err = appendMutation(chs[1], "OrderLineItem", ol)
						if err != nil {
							return err
						}
					}
					...
					for _, p := range ps {
						...
						err = appendMutation(chs[2], "Payment", p)
						if err != nil {
							return err
						}
					}
			...
			readersCh <- &processJob{
				...
				processFunction: func(j *processJob, chs []chan *spanner.Mutation) error {
					...
					for _, p := range ps {
						...
						err = appendMutation(chs[0], "Product", p)
						if err != nil {
							return err
						}
						for i := pisidx; i < len(pis); i++ {
							...
							err = appendMutation(chs[1], "ProductInfo", pis[i])
							if err != nil {
								return err
							}
						}
						...
						for i := ppsidx; i < len(pps); i++ {
							...
							err = appendMutation(chs[2], "ProductPrice", pps[i])
							if err != nil {
								return err
							}
						}
						...
						for i := pssidx; i < len(pss); i++ {
							...
							err = appendMutation(chs[3], "ProductStock", pss[i])
							if err != nil {
								return err
							}
						}
			...
			readersCh <- &processJob{
				...
				processFunction: func(j *processJob, chs []chan *spanner.Mutation) error {
					gocsv.UnmarshalBytesToCallback(j.files[j.mapping["warehouses"]], func(w *dbWarehouse) error {
						...
						return appendMutation(chs[0], "Warehouse", w)
					})
