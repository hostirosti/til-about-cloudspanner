...
func generateOrders(gcs *GCSclient, wg *sync.WaitGroup) {
	...
	for i := 0; i < *numOrders; i++ {
		...
		for j := 0; j < rand.Intn(10); j++ {
			ol := NewGeneratedOrderLineItem(o.OrderID, int64(rand.Intn(*numProducts)))
			ols = append(ols, ol)
		}

		p := NewGeneratedPayment(o.OrderID)
		ps = append(ps, p)
...
func generateProducts(gcs *GCSclient, wg *sync.WaitGroup) {
	...
	for i := 0; i < *numProducts; i++ {
		...
		pi := NewGeneratedProductInfo(p.ProductID, *infoDataSize)
		pis = append(pis, pi)

		for j := 0; j < rand.Intn(10)+1; j++ {
			pp := NewGeneratedProductPrice(p.ProductID)
			pps = append(pps, pp)
		}

		for j := 0; j < rand.Intn(10)+1; j++ {
			ps := NewGeneratedProductStock(p.ProductID, int64(rand.Intn(*numWarehouses)))
			pss = append(pss, ps)
		}
