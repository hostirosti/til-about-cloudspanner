...
func (ijr *insertJobsRunner) run(chs []chan *spanner.Mutation) {
	...
	go func() {
		...
		for _, ch := range chs {
			wg.Add(1)
			go func(c <-chan *spanner.Mutation, wg *sync.WaitGroup) {
				b := []*spanner.Mutation{}
				for m := range c {
					b = append(b, m)
					if len(b) >= ijr.maxBatchSize {
						ijr.flush(b)
						b = make([]*spanner.Mutation, 0)
					}
				}
				if len(b) > 0 {
					ijr.flush(b)
...
func (ijr *insertJobsRunner) flush(b []*spanner.Mutation) {
	tb := make([]*spanner.Mutation, len(b))
	copy(tb, b)
	ijr.jobs <- &job{
		ms: tb,
	}
